 ...
 */
 /*:ja
 * @plugindesc v1.05 (要YEP_SkillCore.js) スキル使用にアイテム消費が必要にできる機能を追加します。
 * @author Yanfly Engine Plugins
 *
 * @param ---一般---
 * @default
 ...
 * @option 量・アイコンと並べる
 * @value 2
 * @desc コスト表示形式。 0 - 無し   1 - アイコン上に量を重ねる
 * 2 - 量・アイコンと並べる
 * @default 2
 *
 * @param Font Size
 ...
 * このプラグインがプラグインリストのYEP_SkillCoreの下にあることを確認してください。
 *
 * このプラグインを使用すると、アイテムを使用してスキルのコストを
 * 設定することができます。
 ...
 * ============================================================================
 *
 * スキルがアイテムをコストとして利用できるようにするには、次のメモタグを使用します。
 * 
 * スキルのメモタグ:
 *   <Item x Cost: y>
 *   <Weapon x Cost: y>
 ...
 *   どの重要アイテムが使用に適しているかを区別できないため、
 *   重要アイテムをアイテムコストとして使用することはできません。 
 *   これらのタグを複数使用している場合、
 *   スキルはリストされているすべてのアイテムを
 *   使用できるようにする必要があります。
 *
 *   <Item Cost: x Potion>
 *   <Item Cost: x Sword>
 ...
 *   これにより、スキルは命名されたアイテム、武器、または防具を x 個必要とします。
 *   同じ名前のアイテムが複数ある場合は、
 ...
 *   重要アイテムをアイテムコストとして使用することはできません。
 *   これらのタグを複数使用している場合、
 *   スキルはリストされているすべてのアイテムを使用できるようにする必要があります。
 *
 * クラス、武器、防具、ステータスのメモタグ:
 *   <Swap Gauge x: Item y>
 *   <Swap Gauge x: Weapon y>
 *   <Swap Gauge x: Armor y>
 *   ゲージ x と交換して、プレイヤー/パーティーが持っている
 *   アイテム、武器、または鎧の量を表示します。
 *   優先順位は、武器、防具、状態、クラス、敵の順です。
 *
 *   <Swap Gauge x: Item Potion>
 *   <Swap Gauge x: Item Sword>
 *   <Swap Gauge x: Item Feather Cap>
 ...
 *   <Armor x Cost: +y>
 *   <Armor x Cost: -y>
 *   y の値で要求されるとき、アイテム、武器、または防具 x のコストを増減します。
 *   アイテム、武器、防具が必要ない場合、この効果はスキルコストには適用されません。
 *
 *   <Item Cost: +x Potion>
 *   <Item Cost: -x Sword>
 *   <Item Cost: +x Feather Cap>
 ...
 *   アイテムの名前を使用したい場合は、上記のメモタグを使用できます。
 *   アイテムがコストとして必要とされる場合、
 *   これは言及されたアイテムのコスト率をx％だけ調整します。
 *   同じ名前のアイテムが複数ある場合は、
 *   メモタグがアイテム、武器、鎧の順で最も高いID値を優先します。
 *
 *   <Replace Type x Cost: Type y>
 *   Typeを 'item'、 'weapon'、'armor'のいずれかに置き換えてください。
 *   これにより、特定の品目に使用されたコストを
 *   別の品目（品目y）と交換することができます。
 *   置き換えは、ステータス、武器、防具、職業、アクターの順に優先されます。
 *
 *   <Replace Potion Cost: Ether>
 *   <Replace Sword Cost: Dagger>
 *   <Replace Feather Cap Cost: Bandana>
 ...
 *   テキスト色 x を使用して指定の色で表示させることができます。
 *
 *   <Item Gauge Text: x>
 ...
 *    cost = user.level;
 *   </Custom Item Cost: name>
 *   名前を使用したい場合、'name'をコストとして使用するアイテムの名前に
 *   置き換えてください。
 *   'cost'変数は、名前付きアイテムがコストとしてどれだけが必要かを決定します。
